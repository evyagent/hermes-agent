...
// EVY fork: the V mark (same path as the EVY panel/landing logo) on a white
// tile, identical in light/dark. Size via className (default size-14).
...
export function BrandMark({ className, ...props }: React.ComponentProps<'span'>) {
  return (
    <span
      className={cn(
        'inline-flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-md bg-white text-black',
        className
      )}
      {...props}
    >
      <svg aria-hidden="true" className="size-[62%]" fill="currentColor" focusable="false" viewBox={MARK_VIEWBOX}>
        <path d={MARK_PATH} />
      </svg>
    </span>
  )
